...
def get_info(info_id: int, db: Session = Depends(connect_db)):
    ...
    """
    result = logic_info.get_info(db=db, info_id=info_id)
...
def update_info(
    ...
    ## Lưu ý: Trường techstack sẽ GHI ĐÈ HẾT nên hãy chắc chắn đủ key bắt buộc và dữ liệu đầy đủ nhất
    """
    result = logic_info.update_info(db = db, target_id = info_id, update_data=update_data)
    return ResponseModel(
        data=result,
