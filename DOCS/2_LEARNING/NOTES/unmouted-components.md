# `🎯 Note về xử lý unmounted components: `

<blockquote>
Khi component  được render, `useEffect` chạy và bắt đầu gọi API. Việc gọi API này mất thời gian (vài trăm ms, có khi vài giây). Trong lúc đang đợi kết quả trả về, nếu người dùng bấm chuyển sang trang khác (route khác) — thì component đã bị **unmount** (React gỡ nó khỏi màn hình, xoá luôn).
  ...
  const fetchData = async () => {
    try {
      ...
      if (isMounted) {
        ...
        setData(response); // chỉ set khi còn "sống"
      }
    } catch (err) {
...
### `Cái gì khiến component unmount** và **cleanup function hoạt động ra sao ?`
...
- **chuyển route** (`/about` → `/contact`) --> rơi vào trường hợp 2 ở trên.
...
<blockquote>
Tóm gọn: unmount = component biến mất khỏi cây render (do cha ngừng render, đổi route, hoặc đổi key). Còn `isMounted` chỉ đơn giản là 1 cái cờ đóng trong bộ nhớ của closure, được React tự "bật/tắt" đúng lúc unmount xảy ra.
</blockquote>
