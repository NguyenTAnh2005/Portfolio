## `🎯 Note Timeline vs Project — khác pattern update ảnh`
...
- Tổng công việc server phải làm **y hệt nhau**, chỉ là tách thành 2 cuộc gọi HTTP thay vì 1.
...
- FE: nếu form Project edit cho sửa cả text lẫn ảnh cùng lúc, chỉ cần gọi tuần tự 2 API khi Save (nếu có file mới thì gọi thêm request ảnh) — không cần Promise.all song song, làm tuần tự để tránh race về old_public_id. ( Khả năng phải tách để quản lý riêng)
